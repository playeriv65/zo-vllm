...
def test_loss_difference():
    ...
    # Load HF model on CPU (for direction metadata)
    hf_model = AutoModelForCausalLM.from_pretrained(
        model_name, torch_dtype=torch.float16, device_map="cpu"
    )
    ...
    # Single sample
    loss_plus_single, loss_minus_single = scorer.score_plus_minus(
        prompts_single, lora_runtime
    )
    c_single = compute_projected_grad(loss_plus_single, loss_minus_single, eps=eps)
    ...
    # Batch of 16
    loss_plus_batch, loss_minus_batch = scorer.score_plus_minus(
        prompts_batch, lora_runtime
    )
    c_batch = compute_projected_grad(loss_plus_batch, loss_minus_batch, eps=eps)
    ...
    if abs(expected_batch_diff) > 1e-7:
        ...
        ratio = 1.0
        print("  Ratio:    1.0 (expected diff is zero)")
