...
def run_manual_baseline(hf_model, tokenizer, prompts, config, random_seed):
    ...
    # Save global RNG state and set seed (aligned with the plain LOZO provider)
    ...
    for name, param in named_params:
        ...
        # V: sample new if step % nu == 0 (step=0 means initialize)
        if step % config["nu"] == 0:
...
def run_vllm_step(direction_provider, scorer, lora_runtime, prompts, random_seed, eps):
...
def compare_directions(baseline_dirs, vllm_dirs):
    ...
    for name in baseline_dirs:
        ...
        if U_match and V_match:
            ...
            if not U_match:
                print(
                    f"    U diff: max={torch.max(torch.abs(U_base - U_vllm.float())).item():.6f}"
                )
            if not V_match:
                print(
                    f"    V diff: max={torch.max(torch.abs(V_base - V_vllm.float())).item():.6f}"
                )
...
def main():
    ...
    print("=== Configuration ===")
    print(f"rank={rank}, eps={eps}, lr={lr}, nu={nu}")
    ...
    print(
        f"Loss1_avg vs Loss+: diff={loss1_diff:.6f} ({'✓' if loss1_diff < 1e-4 else '✗'})"
    )
    print(
        f"Loss2_avg vs Loss-: diff={loss2_diff:.6f} ({'✓' if loss2_diff < 1e-4 else '✗'})"
    )
