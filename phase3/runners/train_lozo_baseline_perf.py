...
class PerfLOZOTrainer(LowRankTrainer):
    ...
    def lowrank_zo_step(self, model, inputs):
        ...
        if (
            ...
            print(
                f"[LOZO] step={measured_index} plus={loss_plus:.6f} "
                f"minus={loss_minus:.6f} c={self.projected_grad:.6f} "
                f"step_s={step_s:.6f}",
                flush=True,
            )
...
def main():
    ...
    parser.add_argument("--profile-mode", choices=["minimal", "instrumented"], default="minimal")
